...
(function (BlackCat) {
    class BuyExchangePurchaseView extends BlackCat.ViewBase {
        ...
        create() {
            ...
            this.tradelogDiv.classList.add("pc_bj", "pc_paylist");
                ...
                if (res.r) {
                    if (res.data && res.data.length >= 1) {
                        if (res.data.length < this.num) {
                            this.isLast = true;
                            this.divRecListsMore.textContent = BlackCat.Main.langMgr.get("paylist_noMore");
                        }
                        else {
                            this.page += 1;
                            this.divRecListsMore.textContent = BlackCat.Main.langMgr.get("paylist_getMore");
                        }
                        yield res.data.forEach(txlist => {
                            var txlistObj = this.objCreate("li");
                            txlistObj.onclick = () => {
                            ...
                            this.ObjAppend(cnts_div, txamount_span);
                            this.ObjAppend(txlistObj, cnts_div);
                            var cnts = BlackCat.Main.viewMgr.payView.getListCnts(txlist);
                            if (cnts) {
                                this.ObjAppend(cnts_div, cnts);
                                var cnts_class = BlackCat.Main.viewMgr.payView.getListCntsClass(txlist);
                                if (cnts_class)
                                    cnts_div.classList.add(cnts_class);
                            }
                            var state = BlackCat.Main.viewMgr.payView.getListState(txlist);
                            if (state)
                                this.ObjAppend(cnts_div, state);
...
(function (BlackCat) {
    class PayExchangeRefundView extends BlackCat.ViewBase {
        ...
        updateBalance() {
            var newBalance = BlackCat.PayExchangeShowWalletView.balance - (Number(this.inputTransferCount.value) + (Number(PayExchangeRefundView.crosschain_fee)));
            BlackCat.PayExchangeShowWalletView.balance = newBalance;
        }
    }
